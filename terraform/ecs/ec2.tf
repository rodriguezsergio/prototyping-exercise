resource "aws_launch_configuration" "ecs-launch-config" {
    name_prefix                 = "ecs-lc-"
    image_id                    = "ami-0a6be20ed8ce1f055"
...
runcmd:
 - echo "ECS_CLUSTER=${aws_ecs_cluster.citrusbyte-cluster.arn}" >> /etc/ecs/ecs.config
    EOF
...
resource "aws_autoscaling_group" "ecs-asg" {
  name                      = "ecs-asg"
  vpc_zone_identifier       = ["${data.aws_subnet.us-east-1a.id}",
